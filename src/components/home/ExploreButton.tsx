...
export function ExploreButton({ placeName, className = "" }: { placeName: string; className?: string }) {
  return (
    <a
      ...
      className={`inline-flex items-center gap-1.5 rounded-full bg-forest-900/90 px-3.5 py-2 text-xs font-semibold text-ivory-50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:bg-gold-600 hover:text-forest-950 ${className}`}
    >
      <Search size={13} />
      Explore
    </a>
